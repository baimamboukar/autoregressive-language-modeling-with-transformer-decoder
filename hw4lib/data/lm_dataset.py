...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        # Take subset
        subset_size = min(len(self.text_files), config.get('subset_size', len(self.text_files))) if 'subset_size' in config else len(self.text_files)
        ...
        for file in tqdm(self.text_files):
            ...
            # Shifted: [SOS, token1, token2, ...] for input to model
            # Golden: [token1, token2, ..., EOS] for targets
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        lengths = torch.tensor([len(seq) for seq in shifted_transcripts])  # (B)

        # Pad sequences (use torch.nn.utils.rnn.pad_sequence and pad with pad_token)
        # pad_sequence expects batch_first=True to get (B, T) shape
