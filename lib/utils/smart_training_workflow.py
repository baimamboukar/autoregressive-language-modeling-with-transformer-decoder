# Smart Training Workflow for P2 - CER < 6% Strategy
import os
import yaml
...
def train_model_variant(config_path: str, epochs: int = 25, experiment_name: str = None) -> str:
    ...
    from lib.utils import create_optimizer, create_scheduler
...
def finetune_best_models(
    ...
    """
    from lib.utils.resume_from_wandb import resume_and_continue_training
...
def smart_training_pipeline():
    ...
    variant_configs = [
        "optimization_configs/config_variant_1_balanced.yaml",
        "optimization_configs/config_variant_2_wider.yaml",
        "optimization_configs/config_variant_3_deeper.yaml",
        "optimization_configs/config_variant_4_less_reduction.yaml",
        "optimization_configs/config_variant_5_small_vocab.yaml"
    ]
    ...
    print("\n🤝 Creating ensemble...")
    from lib.utils.ensemble_asr import ensemble_from_wandb
...
def quick_train_single(config_name: str, epochs: int = 25) -> str:
    ...
    return train_model_variant(f"optimization_configs/config_{config_name}.yaml", epochs)
