...
def ensemble_from_wandb(run_ids: list, temperature: float = 1.0, beam_width: int = 5, length_penalty: float = 0.8):
    ...
    for i, run_id in enumerate(run_ids):
        ...
        try:
            ...
            model_file = wandb.restore('best_model.pth', run_path=f"idlf25/P2/{run_id}")
            config_file = wandb.restore('config.yaml', run_path=f"idlf25/P2/{run_id}")
            arch_file = wandb.restore('model_arch.txt', run_path=f"idlf25/P2/{run_id}")
