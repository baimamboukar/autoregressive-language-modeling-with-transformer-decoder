...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        # CTC inputs need to be in (seq_len, batch_size, features) format for CTC loss
        ctc_inputs = {
            'log_probs': ctc_logits.transpose(0, 1),  # (seq_len, batch_size, num_classes)
            'lengths': x_enc_lengths  # (batch_size,)
        }
